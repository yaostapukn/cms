...
export default function PostDetailPage() {
  ...
  const getDetail = async () => {
    let response = await fetch(`http://127.0.0.1:8000/api/content/${id}`, {
      method: 'GET',
      headers: {
  ...
  return (
    <div className="text__editor__actions">
      ...
      <div onClick={() => makeRequest('PUT')}>Отправить изменения</div>
      <div onClick={() => makeRequest('DELETE')}>Удалить</div>
    </div>
  )
}
